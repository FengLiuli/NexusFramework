...
namespace NexusFramework.GAS.Editor
{
    ...
    public static class LubanConfigLoaderGenerator
    {
        ...
        {
            writer.WriteLine("/// <summary>");
            writer.WriteLine("/// 将 _tables 中所有配置批量注册到 ConfigModel，支持追加覆盖。");
            ...
            writer.Indent++;
            {
                ...
                writer.WriteLine("// 注册所有 GameplayEffect 配置");
                writer.WriteLine("foreach (var effect in _tables.TbgameplayEffect.DataList)");
                writer.WriteLine("{");
                writer.Indent++;
                writer.WriteLine("var config = GetEffectConfig(effect.ID);");
                writer.WriteLine("if (config != null)");
                writer.WriteLine("    configModel.RegisterEffect(effect.ID, config);");
                writer.Indent--;
                writer.WriteLine("}");
                writer.WriteLine("");

                writer.WriteLine("// 注册所有 Ability 配置");
                writer.WriteLine("foreach (var ability in _tables.Tbability.DataList)");
                writer.WriteLine("{");
                writer.Indent++;
                writer.WriteLine("var config = GetAbilityConfig(ability.ID);");
                writer.WriteLine("if (config != null && config.Length > 0)");
                writer.WriteLine("    configModel.RegisterAbility(ability.ID, config);");
                writer.Indent--;
                writer.WriteLine("}");
                writer.WriteLine("");

                writer.WriteLine("// 注册所有 GameplayCue 配置（数组下标 = ID）");
                writer.WriteLine("{");
                writer.Indent++;
                writer.WriteLine("int maxId = 0;");
                writer.WriteLine("foreach (var cue in _tables.TbgameplayCue.DataList)");
                writer.WriteLine("    if (cue.ID > maxId) maxId = cue.ID;");
                writer.WriteLine("var cues = new GameplayCueConfig[maxId + 1];");
                writer.WriteLine("foreach (var data in _tables.TbgameplayCue.DataList)");
                writer.WriteLine("{");
                writer.Indent++;
                writer.WriteLine("var cueTypeName = data.CueLogic.GetType().Name;");
                writer.WriteLine("var param = CreateCueParam(data.CueLogic);");
                writer.WriteLine("");
                writer.WriteLine("int[] FlattenTags(cfg.TagRequirementData? req)");
                writer.WriteLine("{");
                writer.Indent++;
                writer.WriteLine("if (req == null) return Array.Empty<int>();");
                writer.WriteLine("var r = req.Value;");
                writer.WriteLine("var list = new List<int>();");
                writer.WriteLine("if (r.All is { Count: > 0 }) list.AddRange(r.All.Where(x => x > 0));");
                writer.WriteLine("if (r.Any is { Count: > 0 }) list.AddRange(r.Any.Where(x => x > 0));");
                writer.WriteLine("if (r.None is { Count: > 0 }) list.AddRange(r.None.Where(x => x > 0));");
                writer.WriteLine("return list.ToArray();");
                writer.Indent--;
                writer.WriteLine("}");
                writer.WriteLine("");
                writer.WriteLine("cues[data.ID] = new GameplayCueConfig");
                writer.WriteLine("{");
                writer.Indent++;
                writer.WriteLine("CueType = cueTypeName,");
                writer.WriteLine("Param = param,");
                writer.WriteLine("RequiredTags = FlattenTags(data.RequiredTag),");
                writer.WriteLine("ImmunityTags = FlattenTags(data.ImmunityTag)");
                writer.Indent--;
                writer.WriteLine("};");
                writer.Indent--;
                writer.WriteLine("}");
                writer.WriteLine("configModel.RegisterCues(cues);");
                writer.Indent--;
                writer.WriteLine("}");
                writer.WriteLine("");

                writer.WriteLine("// 注册所有 MMC 配置（数组下标 = ID）");
                writer.WriteLine("{");
                writer.Indent++;
                writer.WriteLine("int maxId = 0;");
                writer.WriteLine("foreach (var mmc in _tables.Tbmmc.DataList)");
                writer.WriteLine("    if (mmc.ID > maxId) maxId = mmc.ID;");
                writer.WriteLine("var mmcs = new MMCConfig[maxId + 1];");
                writer.WriteLine("foreach (var data in _tables.Tbmmc.DataList)");
                writer.WriteLine("{");
                writer.Indent++;
                writer.WriteLine("var mmcTypeName = data.MmcLogic.GetType().Name;");
                writer.WriteLine("var param = CreateMmcParam(data.MmcLogic);");
                writer.WriteLine("mmcs[data.ID] = new MMCConfig");
                writer.WriteLine("{");
                writer.Indent++;
                writer.WriteLine("MmcType = mmcTypeName,");
                writer.WriteLine("Param = param");
                writer.Indent--;
                writer.WriteLine("};");
                writer.Indent--;
                writer.WriteLine("}");
                writer.WriteLine("configModel.RegisterMmcs(mmcs);");
                writer.Indent--;
                writer.WriteLine("}");
                writer.WriteLine("");

                writer.WriteLine("// 注册标签层级数据");
                writer.WriteLine("configModel.RegisterTagHierarchy(GetTagHierarchyData());");
            }
            writer.Indent--;
            writer.WriteLine("}");
            writer.WriteLine("");
