...
### 1.1 Entity ↔ GameObject 桥接未注入 ✅ 已清理 (2026-06-11)
...
- `GameplayCueBase` 和 `TargetCatcherBase` 通过 `SetEntityResolver()` 注入
- `TargetCatcherBase` 提供 `ResolveGameObject()` / `ResolveEntity()` 便捷方法；`GameplayCueBase` 提供 `GetTargetAscGameObject()` 辅助方法
...
- **已删除** `EntityGameObjectBindings`（无任何 C# 引用）；`CatchAreaBox3D` 双路径回退已统一
